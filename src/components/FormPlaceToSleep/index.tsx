...
interface FormPlaceToSleepData {
  name: string;
  description: string;
  categories: string[];
  address: string;
...
interface FormPlaceToSleepProps {
  initialData?: PlaceToSleep;
  method: "edit" | "add";
  url: string;
}

const FormPlaceToSleep: React.FC<FormPlaceToSleepProps> = ({
  initialData,
  method,
  url,
}) => {
  ...
  useEffect(() => {
    console.log("FormPlaceToSleep - schedules state updated:", schedules);
  }, [schedules]);
  ...
  const handleSubmit = useCallback(
    async (data: FormPlaceToSleepData) => {
      console.log(data);
      ...
      >
        ...
        <FormStep>
          ...
          <StepContent>
            ...
            <Textarea
              name="description"
              placeholder="Descreva o local, suas acomodações, serviços..."
              {...{ rows: 6 }}
            />
          </StepContent>
        </FormStep>
        ...
        <FormStep>
          ...
          <StepContent>
            ...
            <MultiSelectCategories
              ...
              categoryType={CategoryType.PLACE_TO_SLEEP}
            />
          </StepContent>
        </FormStep>
            ...
            <FileUploadMultiple
              ...
              existingImages={
                initialData?.images?.map((img) => {
                  console.log("FormPlaceToSleep - Existing image:", img);
                  return {
                    key: img.image,
        ...
        <FormActions>
          <BackButton
            type="button"
            onClick={() => navigate("/lugares-para-dormir")}
          >
            <FiArrowLeft size={20} />
            Voltar
          </BackButton>
...
export default FormPlaceToSleep;
